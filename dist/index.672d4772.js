...
const showTotal = ()=>{
    ...
    if (total > 0) summary.style.visibility = "visible";
    else summary.style.visibility = "hidden";
...
const productsInMonths = ()=>{
    ...
    if (selectedProductsQuantity > 0) firstLi.style.visibility = "visible";
    else firstLi.style.visibility = "hidden";
...
const ordersInMonths = ()=>{
    ...
    if (estimentedOrdersInMonths > 0) secondLi.style.visibility = "visible";
    else secondLi.style.visibility = "hidden";
...
const packageSelected = (el)=>{
    const _package = el.target.dataset.package;
    const prize = packages.find((x)=>x.name == _package).value;
    selectedPackage = prize;
    if (selectedPackage > 0) {
        thirdLi.style.visibility = "visible";
        thirdLi.innerHTML = `<span class="item-type">Package</span> <span class="item-calc">${_package}</span><span class="item-price">$${prize}</span>`;
    } else thirdLi.style.visibility = "hidden";
    showTotal();
};
...
icon.addEventListener("click", function() {
    ...
    if (packageDropdown.classList.contains("show")) arrow.style.rotate = "180deg";
    else arrow.style.rotate = "0deg";
});
